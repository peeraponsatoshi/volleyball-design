...
export default () => {
  ...
  const createArcTextElement = (fontSize: number, textStyle = 'transverse', textHollow = false, textValue = t("default.textValue")) => {
    const { centerPoint } = useCenter()
  ...
  const createVerticalTextElement = (fontSize: number, textHollow = false, textValue = t("default.textValue")) => {
    const { centerPoint } = useCenter()
