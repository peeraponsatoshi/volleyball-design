  ...
  <div class="canvas-design-panel">
    ...
    <div class="mt-10">
      <el-row>
        ...
        <el-col :span="11">
          <el-select v-model="sizeMode" @change="changeSizeMode">
            <template #prefix>
              <el-tooltip
                placement="top"
                :hide-after="0"
                :content="t('style.canvasSize')"
              >
                <IconIdCard />
              </el-tooltip>
            </template>
...
/** Apply social / FB pixel presets */
const changeSizeMode = async () => {
  const preset = DesignSizeMode.find((i) => i.id === sizeMode.value);
  if (!preset || !preset.width || !preset.height) return;

  // Social sizes are always pixels
  unitMode.value = 1;
  ...
  workSpaceDraw.set({ width: preset.width, height: preset.height });
  templatesStore.setSize(preset.width * z, preset.height * z, z);
  canvas.renderAll();
  setCanvasSize();
  resetCanvas();
  addHistorySnapshot();
  ElMessage({ type: 'success', message: `ตั้งขนาด ${preset.name}` });
};
